    ...
    <ul class="edit-list">
      ...
      <li class="edit-list-item">
        <main class="edit-list-item-content" @click="setProfile('birthday')">
          ...
          <Datepicker v-else :placeholder="$t(`TUIProfile.请选择出生日期`)" :enableTimePicker="false" :format="format"
            :previewFormat="format" :modelValue="birthday" @update:modelValue="showBirthday" />
        </main>
        <i v-if="isH5" class="icon icon-right"></i>
      </li>
    </ul>
        ...
        <main class="edit-h5-main">
          <ul class="list" v-if="editConfig.type === 'select'">
            <li class="list-item" v-for="(item, index) in editConfig.list" :key="index" @click="choose(item)">
              ...
              <span v-else :class="[editConfig.value === item.type && 'selected']">{{ $t(`TUIProfile.${item.label}`)
              }}</span>
            </li>
          </ul>
          <div class="input" v-else>
            <textarea v-if="editConfig.type === 'textarea'" :placeholder="editConfig.placeholder"
              v-model="editConfig.value"></textarea>
            <Datepicker class="datePicker" :placeholder="$t(`TUIProfile.请选择出生日期`)" v-else-if="editConfig.type === 'date'"
              :enableTimePicker="false" :format="format" :previewFormat="format" :modelValue="birthday"
              @update:modelValue="showBirthday" />
            <input v-else :type="editConfig.type" :placeholder="$t(`TUIProfile.${editConfig.placeholder}`)"
