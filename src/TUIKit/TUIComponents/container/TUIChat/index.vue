<template>
  <div class="TUIChat" :class="[env.isH5 ? 'TUIChat-H5' : '']" v-if="conversationType === 'chat'">
    <header class="TUIChat-header">
      ...
      <TypingHeader :needTyping="needTyping" :conversation="conversation" :messageList="messageList" :inputText="text"
        :inputBlur="inputBlur" :inputCompositionCont="inputCompositionCont" />
      <aside class="setting">
    ...
    <div class="TUIChat-main">
      ...
      <ul class="TUI-message-list" @click="dialogID = ''" ref="messageEle" id="messageEle">
        ...
        <li v-for="(item, index) in messages" :key="index" :id="item?.ID" ref="messageAimID">
          ...
          <MessageBubble v-else-if="!item.isRevoked" :isH5="env.isH5" :data="item" :messagesList="messages"
            :needGroupReceipt="needGroupReceipt" :needReplies="true" :needEmojiReact="isNeedEmojiReact" @jumpID="jumpID"
            @resendMessage="resendMessage" @showReadReceiptDialog="showDialog" @showRepliesDialog="showDialog"
            @dropDownOpen="handleDropDownOpen">
            <MessageText v-if="item.type === types.MSG_TEXT" :data="handleTextMessageShowContext(item)" />
            <MessageImage v-if="item.type === types.MSG_IMAGE" :data="handleImageMessageShowContext(item)"
              :isH5="env.isH5" @uploading="handleUploadingImageOrVideo" @previewImage="handleImagePreview" />
            <MessageVideo v-if="item.type === types.MSG_VIDEO" :data="handleVideoMessageShowContext(item)"
              :isH5="env.isH5" @uploading="handleUploadingImageOrVideo" />
            ...
            <template #dialog>
              <MessageTool :message="item" :needEmojiReact="isNeedEmojiReact" @forwardMessage="forwardMessage"
    ...
    <div class="TUIChat-footer" :class="[isMute && 'disabled', env.isH5 && 'TUIChat-H5-footer']">
      ...
      <div class="memberList" v-if="showGroupMemberList">
        <ul class="memberList-box" ref="dialog">
          ...
          <li class="memberList-box-body" v-for="(item, index) in allMemberList" :key="index"
      ...
      <div class="reply" v-if="reference?.show === 'reply'">
        <div class="reply-box">
          ...
          <div class="reply-box-show">
            <span>{{ reference.message.nick ? reference.message.nick : reference.message.from }}{{ env.isH5 ? ':' : '' }}
            </span>
      ...
      <div class="input">
        <textarea v-show="!isMute" ref="inputEle" id="input-text-area" @paste="pasting" v-model="text"
          :placeholder="$t('TUIChat.请输入消息')" data-type="text" @input="inputChange" @keyup.delete="deleteAt"
          @keydown.enter="sendMseeage" @keypress="geeks" @blur="inputBlur = true" @focus="inputBlur = false"
          @compositionstart="inputComposition = true" @compositionend="compositionEnd" rows="1"></textarea>
        ...
        <div class="reference" v-if="reference?.show === 'reference' && !isMute">
          <div class="reference-box">
            <div class="reference-box-show">
              <span class="reference-box-show-name">{{ reference.message.nick ? reference.message.nick :
                reference.message.from }}:</span>
              <span>{{ reference.content }}</span>
            </div>
            <label class="icon icon-cancel" @click="closeReferenceInEdit"></label>
          </div>
